  ...
  if (type == KEY_PRESSED) {
    ...
    case LEFT_ARROW:
      if (!state->pushed_down) {
        ...
        create_applied(state->scene, (vector_t) { -CIRCLE_MASS * CIRCLE_ACCELERATION, 0 }, ball);
      }
      break;
    case RIGHT_ARROW:
      if (!state->pushed_down) {
...
void initialize_body_list(scene_t *scene) {
    body_t *body = body_init(make_circle(CIRCLE_RADIUS), CIRCLE_MASS, CIRCLE_COLOR);
...
void initialize_force_list(scene_t *scene) {
    create_downwards_gravity(scene, GRAVITATIONAL_ACCELERATION, scene_get_body(scene, 0));
    create_physics_collision(scene, 0.0, scene_get_body(scene, 0), scene_get_body(scene, 1));
