...
void section_three(list_t *shape, vector_t start, vector_t end, size_t num_points){
...
void section_six(list_t *shape, vector_t start, vector_t end, size_t num_points){
  double dx = (end.x - start.x) / num_points;
  vector_t *coord_zero = malloc(sizeof(vector_t));
  *coord_zero = start;
  list_add(shape, coord_zero);
...
void section_eleven(list_t *shape, vector_t start, vector_t end, size_t num_points){
  double dx = (end.x - start.x) / num_points;
  vector_t *coord_zero = malloc(sizeof(vector_t));
  *coord_zero = start;
  list_add(shape, coord_zero);
...
void section_eighteen(list_t *shape, vector_t start, vector_t end, size_t num_points){
  double dx = (end.x - start.x) / num_points;
  vector_t *coord_zero = malloc(sizeof(vector_t));
  *coord_zero = start;
  list_add(shape, coord_zero);
...
void section_twenty_one(list_t *shape, vector_t start, vector_t end, size_t num_points){
  double dx = (end.x - start.x) / num_points;
  vector_t *coord_zero = malloc(sizeof(vector_t));
  *coord_zero = start;
  list_add(shape, coord_zero);
...
void section_thirty_seven(list_t *shape, vector_t start, vector_t end, size_t num_points){
  double dx = (end.x - start.x) / num_points;
  vector_t *coord_zero = malloc(sizeof(vector_t));
  *coord_zero = start;
...
list_t *make_bike_shape(){
  ...
  for (size_t i = 0; i < NUM_SECTIONS; i++){
    if (i == 2){
      section_three(shape, COORDS[i], COORDS[i + 1], PROPORTIONS[j] * BIKE_NUM_POINTS);
      j++;
    }
    if (i == 5){
      section_six(shape, COORDS[i], COORDS[i + 1], PROPORTIONS[j] * BIKE_NUM_POINTS);
      j++;
    }
    else if (i == 9){
      ...
      section_eleven(shape, COORDS[i], COORDS[i + 1], PROPORTIONS[j] * BIKE_NUM_POINTS);
      j++;
    }
    else if (i == 11){
      ...
      section_eighteen(shape, COORDS[i], COORDS[i + 1], PROPORTIONS[j] * BIKE_NUM_POINTS);
      j++;
    }
    else if (i == 20){
      section_twenty_one(shape, COORDS[i], COORDS[i + 1], PROPORTIONS[j] * BIKE_NUM_POINTS);
      j++;
    }
    else if (i == 34){
      ...
      section_thirty_seven(shape, COORDS[i], COORDS[i + 1], PROPORTIONS[j] * BIKE_NUM_POINTS);
    }
    else {
      vector_t *coord = malloc(sizeof(vector_t));
      *coord = COORDS[i];
