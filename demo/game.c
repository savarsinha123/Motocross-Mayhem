...
const vector_t COORDS[NUM_SECTIONS] = {
    ...
    (vector_t){-1.05, 1.191},    
    (vector_t){-1.1, 1.412},  
    (vector_t){-0.8752, 2.7991}, 
    (vector_t){-0.35, 2.336},   
    (vector_t){0.417, 1.9778},   
    (vector_t){0.6441, 1.9888},  
    (vector_t){0.8, 1.8933},     
    (vector_t){0.94, 1.5667},    
    (vector_t){-1.788, 0.8834},  
    (vector_t){-8.6, 1.0551},    
    (vector_t){-4.306, -0.4199}, 
    (vector_t){-6.58, -1.2912},  
    (vector_t){-6.42, -1.7088},  
    (vector_t){-5.611, -1.3999}, 
    (vector_t){-5.772, -0.9804}, 
    (vector_t){-5.345, -0.8174}, 
    (vector_t){-5.185, -1.236},  
    (vector_t){-3.99, -0.7786},  
    (vector_t){-3.752, -1.1},    
    (vector_t){-1.6274, -1.1},   
    (vector_t){-0.6476, -1.1},   
    (vector_t){0.2912, -1.1},    
    (vector_t){1.53, 0.1899},    
    (vector_t){2.2932, -1.5851},
    (vector_t){2.7068, -1.4149},
    (vector_t){2.364, -0.616}, 
    (vector_t){1.9514, 0.7936},
    (vector_t){1.771, -0.3737},
    (vector_t){2.184, -0.1961},
    (vector_t){1.9708, 0.3014},  
    (vector_t){4.5, -0.09585}, 
    (vector_t){3.5, 0.7268}, 
    (vector_t){1.69, 0.9567}, 
    (vector_t){1.22, 2.0501}, 
    (vector_t){0.741, 2.3056}, 
    (vector_t){0.5566, 2.4422}, 
    (vector_t){-0.2, 2.78},     
    (vector_t){-0.83, 3.595},   
    (vector_t){-0.86, 3.7},     
    (vector_t){-0.9148, 3.748}, 
    (vector_t){-0.3925, 3.675}, 
    (vector_t){-0.25, 3.96},    
    (vector_t){-0.37, 3.96},    
    (vector_t){-0.7, 4.05},     
    (vector_t){-0.8, 4.19},     
    (vector_t){-0.71, 4.703},   
    (vector_t){-0.5529, 4.904}, 
    (vector_t){0.0861, 4.91},  
    (vector_t){-0.4122, 5.228}, 
    (vector_t){-1.701, 3.858}, 
    (vector_t){-1.587, 3.842}, 
    (vector_t){-2.125, 0.9167}, 
    (vector_t){-0.5, 0.988}, 
};
...
list_t *make_star_shape(size_t inner_radius, size_t outer_radius, size_t num_points, double scale_factor) {
...
body_t *make_star(state_t *state, double scale_factor) {
  list_t *shape = make_star_shape(STAR_INNER_RAD, STAR_OUTER_RAD, STAR_NUM_POINTS, scale_factor);
  body_type_t *type = malloc(sizeof(*type));
  *type = STAR;
...
body_t *make_rectangle_with_info(double width, double height, rgb_color_t color, void *info, free_func_t freer) {
  ...
  *top_right = (vector_t){width, height};
  ...
  body_t *rectangle = body_init_with_info(rectangle_shape, BUTTON_MASS, color, info, freer);
  return rectangle;
}
...
void initialize_body_list(state_t *state, track_t make_track) {
  ...
  *finish_type = FINISH;
  body_t *finish = make_rectangle_with_info(FINISH_WIDTH, FINISH_HEIGHT, WHITE, finish_type, free);
  vector_t centroid = (vector_t) { state->goal + 0.5 * FINISH_WIDTH, 0.5 * FINISH_HEIGHT};
...
void kill_powerup(state_t *state) {
  ...
  create_drag(state->scene, DRAG * ACCELERATION_FACTOR * 20, scene_get_body(state->scene, 0));
...
void on_key(state_t *state, char key, key_event_type_t type, double held_time) {
  ...
  if (type == KEY_PRESSED) {
    ...
    case RIGHT_ARROW:
      if (!state->pushed_down) {
        state->pushed_down = true;
        create_applied(state->scene, (vector_t){BIKE_MASS * BIKE_ACCELERATION * cos(angle), sin(angle)},
            bike);
        state->sound = ACC;
        state->sound_changed = true;
      } else {
...
void make_button(state_t *state, char *string, size_t font_size,
  ...
  body_t *button = make_rectangle(BUTTON_SCALING_FACTOR * dim.x, BUTTON_SCALING_FACTOR * dim.y, button_color);
  vector_t centroid =
      (vector_t){position.x + dim.x / CENTROID_SCALING_FACTOR, position.y - dim.y / CENTROID_SCALING_FACTOR};
      ...
      for (size_t i = 0; i < list_size(state->button_list); i++) {
        ...
        if (collision.collided) {
          ...
          case 0:
            ...
            create_powerup_collision(state, scene_get_body(state->scene, 0), scene_get_body(state->scene, scene_bodies(state->scene) - 1));
            sdl_on_key(on_key);
            sdl_on_mouse(NULL);
            state->clock = START_TIME;
            break;
